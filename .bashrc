# ~tL.bashrc: executed by bash(1) for non-login shells.
# see /usr/share/doc/bash/examples/startup-files (in the package bash-doc)
# for examples

# Need to install git automaticly so that I can clone things from it
...
if [ ! -d ~/.git_repos/powerline ]; then
  ...
  mkdir -p ~/.git_repos
  git clone https://github.com/powerline/powerline.git ~/.git_repos/powerline
fi
if [ ! -d ~/.git_repos/powerline/build/ ]; then
  echo "- Building Powerline"
  #this only works on ubuntu add OS checker later
...
if [ ! -L ~/.local/bin/powerline ] && [ ! -e ~/.local/bin/powerline ] && [ ! -f ~/.local/bin/powerline ]; then
...
# Powerline (if running on a slower computer comment out this block)
...
# tmux dev alias
alias dev1='cd ~/.git_repos/ListAndNotes/ && tmux'
alias dev2='cd ~/.git_repos/RematchGaming/ && tmux'
